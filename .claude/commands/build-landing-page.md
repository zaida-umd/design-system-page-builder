...
9. **Footer** — `<umd-element-footer>` with logo, department name, address, phone, email, and social links. For the footer `slot="logo"`, use a department-specific logo if available. If no external logo is accessible, fall back to `../images/logos/footer-logo.svg` (the UMD "Fearlessly Forward" lockup).
