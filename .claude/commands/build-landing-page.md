...
9. **Footer** — `<umd-element-footer data-display="visual">` with logo and campus background image. Always use `../images/logos/footer-logo.svg` for `slot="logo"` and `../images/large/campus/footer-campus.jpg` for `slot="image"`. Do not add contact info, address, or social links to the visual footer — the visual variant renders the logo and image only.
