...
9. **Footer** — `<umd-element-footer>` with logo, department name, address, phone, email, and social links. For the footer `slot="logo"`, always default to `../images/logos/footer-logo.svg` (the UMD "Fearlessly Forward" lockup on a dark background). Do not attempt to use an external department logo URL — external logos are often on light backgrounds or blocked by hotlink protection and will not render correctly in the footer.
