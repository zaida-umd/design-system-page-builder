...
/* image-expand needs explicit width: 100% — without it, the host
   ...
   NOTE: does NOT support data-theme — content slot text must be
   explicitly styled white (color: white) since it overlays a dark
   image. Shadow DOM applies max-width: 1600px internally on the
   text-lock — do NOT set max-width on the host. See RULES.md §17. */
